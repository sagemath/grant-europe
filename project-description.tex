...
\begin{itemize}
\item Distribution improvements: modularization, integration in the
  standard Linux distributions, better Windows support;
\item Documentation improvements: overview, thematic tutorials, cross links;
\item Workflow improvements: continuous integration, ...
\item Deployment of preconfigured virtual machines for Sage as a cloud
  service, in particular with the StratusLab infrastructure.
\item User interface improvements: interactive widgets, robustness;
\item Development of a repository of notebooks;
\item Community animation: training workshops, developers meetings, missions;
\item Infrastructure work;
\item And of course more math features according to the needs.
\item \TODO{Expand!}
\end{itemize}
